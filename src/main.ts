...
function getDataDir(): string {
  return join(process.env.HOME || process.env.USERPROFILE || process.cwd(), '.wechat-claude-code');
}

function getStartCommandHint(): string {
  return process.platform === 'win32' ? 'node dist/main.js start' : 'npm run daemon -- start';
}
...
async function runSetup(): Promise<void> {
  ...
  while (true) {
    ...
    // Open with the system default image viewer when available
    const opened = openFile(QR_PATH);
    if (opened) {
      console.log('已打开二维码图片，请用微信扫描：');
    } else {
      console.log('无法自动打开二维码图片，请手动打开后扫码：');
    }
  ...
  console.log(`运行 ${getStartCommandHint()} 启动服务`);
}
...
async function runDaemon(): Promise<void> {
  ...
  if (session.state !== 'idle') {
    logger.warn('Resetting stale session state on startup', { previousState: session.state });
    session.state = 'idle';
  }
...
async function handleMessage(
  ...
  if (session.state === 'waiting_permission') {
    ...
    const allowReplies = new Set(['y', 'yes', '允许', '同意', '确认', '是']);
    const denyReplies = new Set(['n', 'no', '拒绝', '取消', '否']);

    if (allowReplies.has(normalized)) {
      permissionBroker.resolvePermission(account.accountId, true);
      await sender.sendText(fromUserId, contextToken, '✅ 已允许');
    } else if (denyReplies.has(normalized)) {
      permissionBroker.resolvePermission(account.accountId, false);
      await sender.sendText(fromUserId, contextToken, '❌ 已拒绝');
    } else {
      await sender.sendText(fromUserId, contextToken, '正在等待权限审批，请回复 y / yes / 允许 / 同意，或 n / no / 拒绝 / 取消。');
    }
    return;
  }
  ...
  try {
    ...
    const queryOptions: QueryOptions = {
      ...
      onPermissionRequest: async (toolName: string, toolInput: string) => {
        const decision = decidePermission(toolName, toolInput);
        if (decision.allow) {
          logger.info('Permission auto-allowed', { toolName, reason: decision.reason });
          return true;
        }

        logger.info('Permission requires manual approval', { toolName, reason: decision.reason });
